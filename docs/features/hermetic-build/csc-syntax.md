...
Without `invocation`, `Csc` calls `resolve`, which turns the settings below into a
...
| `invocation` | `Lock.Project` | switches to invocation mode, see below | `None` |
...
composed mode -- invocation mode ignores them (see below). The argument list `resolve` builds is,
...
```fsharp
do! csc { invocation mapped }   // mapped : Lock.Project
```

`invocation` sets `CscSettingsType.Invocation`; when it is `Some project`, `Csc` skips `resolve`
...
first, and only then runs `csc { invocation mapped }` -- the runner itself does not `need` the
...
- A compiler pinned via `Microsoft.Net.Compilers.Toolset` rather than the SDK's own `csc.dll` --
  the import already reads `CscToolPath`/`CscToolExe`, but the runner does not yet special-case
  that source. See `tracker.md`.
