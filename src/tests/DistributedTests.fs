...
[<TestFixture>]
type ``Distributed and resource tests``() =
    ...
    // An in-memory fake of Qualhalla's distributed executor: a run-scoped result store keyed
    ...
    let fakeExecutor () =
        ...
        let executor : DistributedExecutor<ExecContext> =
    ...
    [<Test>]
    member _.``distributed rules are never bounded by the CPU thread pool``() =
        // A distributed rule must NOT consume a local CPU slot — the whole task runs
        // detached. With only 2 CPU slots and no dispatch budget, all 16 distributed
        ...
        let executor : DistributedExecutor<ExecContext> = fun _ctx _targets runBody -> runBody ()
        ...
        let eng = b {
            rules [ for i in 1..n -> (sprintf "dt%d" i => body) |> distributed executor ]
            start
        }
        ...
        Assert.AreEqual(n, !peak,
            sprintf "distributed rules must not be capped by CPU slots (peak was %d, expected %d)" !peak n)

    [<Test>]
    member _.``distributed cache miss runs the body exactly once``() =
        ...
        let eng = b {
            rule (("dt" => body) |> distributed executor)
            start
        }
    ...
    [<Test>]
    member _.``concurrent distributed demands for one identity deduplicate to a single execution``() =
        ...
        let eng = b {
            rules [
                ("dt1" => body) |> distributed executor
                ("dt2" => body) |> distributed executor
            ]
            start
        }
    ...
    [<Test>]
    member _.``distributed cache hit returns stored result without running the body``() =
        let executor, bodyRunCount, store, key = fakeExecutor ()
